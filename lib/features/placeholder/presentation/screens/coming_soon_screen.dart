...
class ComingSoonScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: ValueKey('coming_soon_scaffold_${title.replaceAll(' ', '_').toLowerCase()}'),
      appBar: AppBar(
        key: ValueKey('coming_soon_app_bar_${title.replaceAll(' ', '_').toLowerCase()}'),
        title: Text(title),
        centerTitle: true,
        leading: IconButton(
          key: ValueKey('coming_soon_back_${title.replaceAll(' ', '_').toLowerCase()}'),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  key: ValueKey('coming_soon_icon_container_${title.replaceAll(' ', '_').toLowerCase()}'),
                  width: 100,
                  height: 100,
                ...
                OutlinedButton.icon(
                  key: ValueKey('coming_soon_back_button_${title.replaceAll(' ', '_').toLowerCase()}'),
                  onPressed: () {
                    if (context.canPop()) {
                      context.pop();
                    } else {
